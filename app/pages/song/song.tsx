...
const Song = () => {
	...
	useEffect(() => {
		const play = async () => {
			await TrackPlayer.getActiveTrack().then((track) => { console.log(track) })
			...
			await TrackPlayer.getPlaybackState().then((state) => {console.log(state)})
			await TrackPlayer.getVolume().then((volume) => {console.log(volume)})
			await TrackPlayer.play();
		}
		play()
	}, [])
	...
	return <Layout>
		...
		<Title text={songData.title} size={30} classNames={"mx-auto mb-4"}/>
		<UImage source={songData.album.cover_big} classNames={'rounded-full mx-auto'} width={300} height={300}/>
